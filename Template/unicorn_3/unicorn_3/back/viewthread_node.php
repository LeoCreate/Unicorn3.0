...
<tr>
	...
	<td class="pls" rowspan="2">
		<div id="favatar$post[pid]" class="pls favatar" style="padding-top:10px;">
				...
				<div class="p_pop blk bui card_gender_$post['gender']" id="userinfo$post[pid]" style="display: none; {if $_G['setting']['authoronleft']}margin-top: -11px;{/if}">
					...
					<div class="i y">
						...
						<div class="imicn">
							...
							<!--{if $post['taobao'] && !$post['privacy']['profile']['taobao']}--><a href="javascript:;" onclick="window.open('http://amos.im.alisoft.com/msg.aw?v=2&uid='+encodeURIComponent('$post[taobaoas]')+'&site=cntaobao&s=2&charset=gbk')" title="{lang taobao}"><img src="{IMGDIR}/taobao.gif" alt="{lang taobao}" /></a><!--{/if}-->
    ...
    <div class="qing_ts">					
					<span id="thread_subject">$_G[forum_thread][subject]</span><a href="forum.php?mod=viewthread&tid=$_G[tid]$fromuid" onclick="return copyThreadUrl(this, '$_G[setting][bbname]')" {if $fromuid}title="{lang share_url_copy_comment}"{/if} style="font-size:12px; font-weight:400; color:#999; margin-left:5px;">[{lang share_url_copy}]</a>
				</div>
                <!--{/if}-->
		<div class="pi" style="position:relative;">
			<!--{if !IS_ROBOT}-->
				<!--{if !$postcount && !$_G['forum_thread']['archiveid'] && $post['first'] }-->
					<div class="qing_v_tj">
						<span class="qing_vt_num"><img src="{$_G['style']['styleimgdir']}/view.png" align="absmiddle"> &nbsp;$_G[forum_thread][views] &nbsp;&nbsp;<img src="{$_G['style']['styleimgdir']}/re.png" align="absmiddle">&nbsp;	$_G[forum_thread][allreplies]</span>
					</div>
				<!--{/if}-->
				<!--{if $post['warned']}-->
			...
			<div class="pti">
				...
				<!--{if !IS_ROBOT && !$_G['forum_thread']['archiveid'] && $post['first'] }-->
					...
					<!--{if !$rushreply}-->
						<!--{if $ordertype != 1}-->
							<a href="forum.php?mod=viewthread&tid=$_G[tid]&extra=$_GET[extra]&ordertype=1"  class="show">{lang post_descview}</a>
						<!--{else}-->
